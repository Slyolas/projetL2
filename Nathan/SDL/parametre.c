...
void redimensionner_fenetre(SDL_Window* window, SDL_Rect* ongletSon, SDL_Rect* ongletTouches, SDL_Rect* rectToucheAvancer, SDL_Rect* rectToucheReculer, SDL_Rect* rectToucheSauter, SDL_Rect* rectVolumeMusique, SDL_Rect* rectVolumeEffets, SDL_Rect* rectBoutonSon, int largeur, int hauteur);
...
void redimensionner_fenetre(SDL_Window* window, SDL_Rect* ongletSon, SDL_Rect* ongletTouches, SDL_Rect* rectToucheAvancer, SDL_Rect* rectToucheReculer, SDL_Rect* rectToucheSauter, SDL_Rect* rectVolumeMusique, SDL_Rect* rectVolumeEffets, SDL_Rect* rectBoutonSon, int largeur, int hauteur) {
    ...
    // Réajuster la position et la taille des onglets
    int tabWidth = screenWidth / 4; // Largeur d'un onglet
    ongletSon->x = (largeur - tabWidth * 2) / 2;
    ongletSon->w = tabWidth;
    ongletTouches->x = ongletSon->x + tabWidth;
    ongletTouches->w = tabWidth;

    // Réajuster la position et la taille des rectangles des touches
    int positionX = largeur * 0.8; // 80% de la largeur de la fenêtre depuis la gauche
    int rectWidth = tabWidth / 2;
    int rectHeight = 30;
    rectToucheAvancer->x = positionX - rectWidth;
    rectToucheAvancer->w = rectWidth;
    rectToucheReculer->x = positionX - rectWidth;
    rectToucheReculer->w = rectWidth;
    rectToucheSauter->x = positionX - rectWidth;
    rectToucheSauter->w = rectWidth;
    rectToucheAvancer->h = rectToucheReculer->h = rectToucheSauter->h = rectHeight;

    // Réajuster la position et la taille des rectangles de volume sonore
    rectHeight = 20;
    rectVolumeMusique->x = positionX - rectWidth;
    rectVolumeMusique->w = rectWidth;
    rectVolumeMusique->h = rectHeight;
    rectVolumeEffets->x = positionX - rectWidth;
    rectVolumeEffets->w = rectWidth;
    rectVolumeEffets->h = rectHeight;

    // Réajuster la position et la taille du bouton de son
    rectBoutonSon->x = 50;
    rectBoutonSon->y = 200;
}
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow("Paramètres du jeu", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_RESIZABLE);
    ...
    parametres.volumeMusique = 50;
    parametres.volumeEffets = 50;
    parametres.toucheAvancer = SDLK_RIGHT; //Touche par défaut pour avancer
    parametres.toucheReculer = SDLK_LEFT; //Touche par défaut pour reculer
    parametres.toucheSauter = SDLK_UP; //Touche par défaut pour sauter
    ...
    while (running) {
            ...
            switch (event.type) {
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (ongletActif == ONGLET_SON) {
                        ...
                        } else if (SDL_PointInRect(&(SDL_Point){mouseX, mouseY}, &rectBoutonSon)) {
                ...
                if (event.window.event == SDL_WINDOWEVENT_RESIZED) {
                    ...
                    redimensionner_fenetre(window, &ongletSon, &ongletTouches, &rectToucheAvancer, &rectToucheReculer, &rectToucheSauter, &rectVolumeMusique, &rectVolumeEffets, &rectBoutonSon, largeur, hauteur);
                }
                break;
            }
        }
	    ...
	    int tabWidth = 320; // Largeur d'un onglet
	    ...
	    ongletSon = (SDL_Rect){(screenWidth - tabWidth * 2) / 2, 0, tabWidth, 50};
	    ongletTouches = (SDL_Rect){ongletSon.x + tabWidth, 0, tabWidth, 50};
        ...
        if (ongletActif == ONGLET_TOUCHES) {
            ...
            rectToucheAvancer = (SDL_Rect){positionX - rectWidth, rectFenetre.h / 2 - 50, rectWidth, rectHeight};
            rectToucheReculer = (SDL_Rect){positionX - rectWidth, rectFenetre.h / 2, rectWidth, rectHeight};
            rectToucheSauter = (SDL_Rect){positionX - rectWidth, rectFenetre.h / 2 + 50, rectWidth, rectHeight};
        ...
        // Dessiner les rectangles pour les boîtes de modification du son uniquement dans l'onglet du son
            ...
            rectVolumeMusique = (SDL_Rect){positionX - rectWidth, rectFenetre.h / 2 - 20, rectWidth, rectHeight};
            rectVolumeEffets = (SDL_Rect){positionX - rectWidth, rectFenetre.h / 2 + 70, rectWidth, rectHeight};
            SDL_SetRenderDrawColor(renderer, 200, 200, 200, 255); // Couleur grise
            ...
            afficherParametresSon(renderer, font, parametres, hautParleurActif, hautParleurDesactive, rectBoutonSon, sonActif, rectFenetre, rectWidth);
